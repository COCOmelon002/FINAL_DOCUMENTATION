...
def list_report():
    ...
    print(username)
    cursor.close()
    return render_template('list_report.html',report=report,username=username)
...
def get_employee_blocks(df):
    cols = df.shape[1] // 3
    return [
        df.iloc[:, :cols],
        df.iloc[:, cols:cols * 2],
        df.iloc[:, cols * 2:cols * 3]
    ]
...
def get_name(block):
    for _, row in block.iterrows():
        ...
        if 'Name' in values:
            return values[values.index('Name') + 1].strip()
    return None

def get_attendance(block):
    idx = block[
        block.apply(lambda r: r.astype(str).str.contains('dd/ww').any(), axis=1)
    ].index

    if idx.empty:
        return []

    rows = block.iloc[idx[0] + 2:].fillna(' ').reset_index(drop=True)
    result = []

    for _, row in rows.iterrows():
        r = row.astype(str).tolist()
        date = r[0].strip()
        if not date:
            continue
        ...
        if any('Absence' in x for x in r):
            record['AM In'] = 'Absence'
        elif all(x.strip() == '' for x in r[1:]):
            record['AM In'] = 'No Record'
        else:
            record['AM In'] = r[1] if len(r) > 1 else ''
            record['AM Out'] = r[2] if len(r) > 2 else ''
            record['PM In'] = r[3] if len(r) > 3 else ''
            record['PM Out'] = r[4] if len(r) > 4 else ''
            record['OT'] = r[5] if len(r) > 5 else ''

        result.append({'date': date, 'record': record})
    print(result)
    return result
...
def upload_attendance():
    ...
    for sheet in sheet_names:
        ...
        df = df.fillna(' ').dropna(how='all').dropna(how='all', axis=1)

        for block in get_employee_blocks(df):
            name = get_name(block)
            print("name : ", name)
            if not name:
                skipped += 1
            ...
            for att in get_attendance(block):
                date = att['date']
                r = att['record']
                ...
                try:
                    cur.execute(
                        ...
                        (user_id, date, am_in, am_out, pm_in, pm_out, ot,LastName)
                    ...
                    print(f'Insert error: {name} {date}', e)
                    skipped += 1
    ...
    flash(
        f'Attendance uploaded — Inserted: {inserted}, Skipped: {skipped}',
        'success'
    )
    return redirect(url_for('admin_record'))
...
def download_attendance():
    ...
    writer.writerow(['User ID','Last Name', 'Date', 'AM In', 'AM Out', 'PM In', 'PM Out', 'OT'])
